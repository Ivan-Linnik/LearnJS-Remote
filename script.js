...
// рекурсия - это приём программирования, полезный, когда задача может быть естествеено разделена на несколько
...
// третья ветвь (else) - представляет pow(x, n) как x * pow(x, n - 1), эта ветвь является шагом рекурсии, вней
...
// предыдущий вызов.
// Глубина рекурсии равна максимальному числу контекстов, хоранимых в стеке.
...
// так как более экономна, так использует один контекст выполнения, при этом объём затрачивания памяти фиксированный на все итерации
// внутри функции.
...
// Самое важное - чтобы используемая структура данных соответствовала моей задачи в части производительности и удобства.
...
// }
// printReverseList(list);
// рекурсивное решение делает тоже самое - проходит список, запоминает элементыв цепочке вложенных вызовов, а затем выводит их.

// Остаточные параметры и оператор расширения

// вызвать функцию можно слюбым количесвтом параметроваргументов, не зависимо от того, как она была определена
// function sum(a, b) {
//     return a + b;
// }
...
// остаточные параметры должны располагать в конце! иначе это вызовет ошибку
...
// в JavaScript e каждого блока кода, функции, переменной есть связанный с ними скрытый объект - лексическое окружение.
// Объект лексического окружения состоит из двух частей: Envorimenr record - хранит все локальные свойства, как переменные,
...
// на лексическое место места, где они были созданы. Проще говоря - Envoriment - это ссылка на лексическое окружение места,
...
// одну общую глобальную область, и скрипты, ссылаясь на одну и ту же переменную могу менять её значение в глобальной области,
