...
// чтобы чделать объект итерируемом асинхронно:
...
// Создам итерируемый объект, но теперь он будет возвращать значения асинхронно, по одному в секунду

// let range = {
//     from: 1,
//     to: 5,

//     // for await of вызывает метод 1 раз в самом начале
//     [Symbol.asyncIterator]() {
//         // возвращает объект-итератор: далее for await of работает только с этим объектом, запрашивая у него следующие значения
//         // вызовом next()
//         return {
//             current: this.from,
//             last: this.to,
...
//                 // автоматически оборачивается в промис посредством async
...
// Опрератор расширения не работает асинхронно! - функции, которые требуют обычных синхронных итераторов не работают с асинхронными.
// Ведь они ожидают Symbol.iterator, а не Symbol.asyncIterator.


// Асинхронные генераторы
// Повторю, как они выглядят и работают, и что они перебираемы.
// function* genSequence(start, end) {
//     for (let i = start; i <= end; i++) {
//         yield i;
//     }
...
// let generator = genSequence(1, 5);

// for (let val of generator) {
//     console.log(val); // 1 2 3 4 5
// }

// В обычных генераторах я не могу использовать await. Все значения должны поступать синхронно: в for of нет места для задержки,
// это синхронная конструкция.
// Но если мне нужно использовать await в теле генератора, на пример для сетевых запросов, то мне нужно просто добавить async
// в начале конструкции генератора:

// async function* genAny(start, end) {
//     for (let i = start; i <= end; i++) {
//         await new Promise(resolve => setTimeout(resolve, 1e3));
//         yield i;
//     }
...
// Технически асинхронный генератор отличается от синхронного ещё тем, что метод generator.next() теперь тоже является асинхронным и
// и возвращает промис.
// Из обычного генератора я могу получить значения так: result = generator.next(), для асинхронного запись такая:
// result = await generator.next()
// (async () => {
//     let generator = genAny(9, 12);
//     console.log(await generator.next()); // {value: 9, done: false}
//     console.log(await generator.next()); // {value: 10, done: false}
...
// Для [Symbol.iterator] обычная практика возвращать генератор, а не обычный объект, как из прошлого примера:
// let range = {
//     from: 3,
//     to: 5, 
...
// Если хочу сделать его асинхронно перебираемым, то нужно сделать так:
// let range = {
//     from: 5,
//     to: 7,
...
//         for (let value = this.from; value <= this.to; value++) {
//             // добавил задержку между значениями, ожидание
//             await new Promise(resolve => setTimeout(resolve, 1e3));
//             yield value;
//         }
//     }
...
//         console.log(val); // 5 6 7 - с задержкой 1 сек
//     }
// }) ();


// Пример из реальной практики
// Есть много сервисов, которые возвращают данные постранично. Например, когда нужен список пользователей или товаров,
// запрос возвращает определённое их количество и URL следующей страницы. Например GitHub позволяет получать список коммитов
// с разбивкой по страницам.

// Мне нужно сделать запрос на URL в виде https://api.github.com/repos/<repo>/commits.
// В ответ придёт json с 30 коммитами, а также с сылкой на следующую страницу в заголовке Link.
// Затем можно использовать эту ссылку для следующего запроса, чтобы получить ещё коммиты и так далее.
// Но предлагается иметь просто объект с коммитами, которые можно перебирать, вместо сложного взаимодействия описанного выше.

// Создам функцию, котрая будет получать коммиты, делая запросы всякий раз, когда это необходимо. Пусть она сама разбирается с
// тем, что касается нумерации страниц, для меня это будет просто for...of:
// async function* fetchCommits(repo) {
//     let url = `https://api.github.com/repos/${repo}/commits`;

//     while (url) {
//         const response = await fetch(url, {
//             headers: { 'User-Agent': 'My script' } // github требует заголовок user-agent
//         });

//         const body = await response.json(); // ответ в формате json (массив коммитов)

//         // ссылка на следующую страницу находится в заголовке, извлекаю её
//         let nextPage = response.headers.get('Link').match(/<(.*?)>; rel="next"/);
//         nextPage = nextPage && nextPage[1];

//         url = nextPage;

//         for (let commit of body) { // буквально - вернуть коммиты один за другим, до окончания страницы
//             yield commit;
//         }
//     }
// }
...
//     for await (const commit of fetchCommits('Ivan-Linnik/LearnJS-Remote')) {
//         console.log(count, commit.commit.message, '|', commit.author.login);

//         if (++count == 10) { // остановить на 100 коммитах
//             break;
//         }
//     }
// })();
...
// Что произошло:
// 1. Я использовал метод фетч браузера для загрузки с удалённого УРЛ. Он позволяет при необходимости добавлять авторизацию и другие
// заголовки. В этом случае гитхаб требует юзер-агент.
// 2. Результат фетч обрабатывается как джейсон - это метод присущий фетч.
// 3. Нужно получить УРЛ следующей страницы из заголовка ответа Link. Он имеет специальный формат, поэтому в книге используется
// регулярное выражение. УРЛ следующей страницы генерируется самим гихаб, он может выглядеть так:
// https://api.github.com/repositories/93253246/commits?page=2.
// 4. Затем я вывожу все полученные коммиты, а когда они закончатся, сработает следующая итерация while(url), которая сделает ещё
// один запрос.
// Внутренняя механика постраничных запросов не видна. Для меня это асинхронный генератор, который просто возвращает коммиты.